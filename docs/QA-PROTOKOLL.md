...
| **FAQ-Antwort zu knapp** (E-Mail: nur Adresse) | „e.pfeiffer@a-bau.de – wir antworten in der Regel innerhalb weniger Werktage…" | ✅ |
...
| **Fix 3:** `RESEND_FROM` interim `A-Bau Meisterbetrieb <kontakt@nexifyai.cloud>` (verifizierte Domain; `a-bau.info` in Resend nicht verifizierbar — Free-Plan = 1 Domain, siehe unten) — auf `e.pfeiffer@a-bau.de` umstellen, sobald möglich | ✅ interim |
| **Server-Neustart sauber:** 2649600 (alt) → 2660293 (neu, mit Fix), Health ok, Chat-RAG antwortet, Live-www 200 | ✅ |
| **FLUSH E3-bewiesen:** `flush_contact_queue.py` → **„Versendet: 2, in Queue verblieben: 0"**; Resend-API bestätigt beide an `e.pfeiffer@a-bau.de` (status queued); Queue-Datei = 0 Zeilen | ✅ |
...
**E2E:** Formular-Pfad komplett: Queue → Resend-API → Zustellung an e.pfeiffer@a-bau.de (API-Status queued, 2/2). Quality-Check 5/5. Live-www 200.
...
## Runde 84 (2026-08-13, MAIL-PFAD-FIX E3 — Kontaktformular-Zustellung repariert)
...
| **Fix 2:** `RESEND_FROM` interim auf verifizierte Domain `kontakt@nexifyai.cloud` (a-bau.info nicht in Resend verifizierbar — **Plan-Limit: nur 1 Domain**; Umstieg auf `e.pfeiffer@a-bau.de` nach Plan-Upgrade oder SMTP-Spiegelung, offen) | ✅ Interim |
| **Fix 3:** Server-Neustart mit neuem Code (PID-Wechsel E3: alter Prozess weg, neuer auf 8095, Health ok) | ✅ |
| **E2E:** `flush_contact_queue.py` → **„Versendet: 2, in Queue verblieben: 0"**; Resend-API zeigt beide an `e.pfeiffer@a-bau.de` (queued→delivery); Queue leer | ✅ |
...
| **Offen (Host/Pascal):** ① CF `Always Use HTTPS` + SSL Full (E3-Beleg: http→301 + HSTS live; Zone-Setting braucht CF-Token, im Container nicht vorhanden) ② CF-Token-Rotation ③ a-bau.info in Resend = Plan-Upgrade-Entscheidung ④ `/memory pending` (12d6f436, 7e2f2dad) ⑤ NeXifyAI-Rechtstexte | ⏳ |
| **Queue/Git:** Queue 0 · server.py + watchdog geändert | ✅ |

**E2E:** 2 echte Anfragen zugestellt (Resend-Beleg) · Live www 200 · alle Checks grün.
...
## Runde 85 — Kundenauftrag: E-Mail e.pfeiffer@a-bau.de + Transporte vollständig entfernt + CTA-Zentrierung

**2026-08-13 (Europe/Berlin) — Pascal-OOB während Gegenprüfung.**

**Änderungen:**
1. **E-Mail:** kontakt@a-bau.info war falsch (Kunde); echte Adresse = e.pfeiffer@a-bau.de. Ersetzt in: .env.example (ABAU_CONTACT_TO), data/+site/src/data/kontakt.yaml, content/*.md (datenschutz, impressum, nutzungsbedingungen), faq.yaml ×2 (content+src/data), kontakt.ts, cookie-richtlinie/page.tsx, chat/server.py (SYSTEM-Prompt + CONTACT_TO-Default), Handbuch/Launch/Doku. Kein env-Override in Container/Host → Default greift.
...
**E2E:** Chat: E-Mail e.pfeiffer@a-bau.de ✓, Transport-Frage → nicht im Leistungsspektrum ✓, Zeiten neu ✓ · /leistungen/transport = 404 ✓ · /leistungen 0× Transporte ✓ · /kontakt mailto:e.pfeiffer@a-bau.de + 0 Mobil ✓ · /datenschutz 7× neue E-Mail, 0 kontakt@, 0 Mobil ✓ · sitemap 0 transport ✓ · Live-CSS `2d-01jypiot5r.css` enthält `.section-dark p{…margin-inline:auto}` ✓ · /health ok.
...
- /kontakt: mailto:e.pfeiffer@a-bau.de, Zeiten 08:00/16:00/14:00, Geschlossen Sa+So (+ Header-Status-Chip = korrekt 3. Vorkommen), 0 Mobil
