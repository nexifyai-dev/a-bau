...
| **Chat 401 (LLM-Fallback)** | **Fall A (häufig, 2026-08-12 behoben): eingefrorener Key.** `_secret()` liest `CUSTOM_API_KEY` beim Prozessstart (Modul-Import); 9Router-Key-Rotation macht die Datei `/home/hermeswebui/.hermes/.env` neuer als den laufenden Prozess → Prozess hat alten Key eingefroren. Fix: Server neu starten (Start-Kommando oben), Key-Gültigkeit vorher prüfen: `curl -X POST http://127.0.0.1:20128/v1/chat/completions -H "Authorization: Bearer $(grep -m1 '^CUSTOM_API_KEY=' /home/hermeswebui/.hermes/.env | cut -d= -f2)" -d '{"model":"ds/deepseek-v4-flash","messages":[{"role":"user","content":"OK"}],"max_tokens":5}'` → 200. Danach Live-Test: `curl -X POST https://a-bau.nexifyai.cloud/api/chat -d '{"message":"test"}'` → echte Antwort statt `401`. | **Fall B (Host-root): Host-Key-Drift** — Host-`/root/.hermes/.env` + `/root/.hermes/hermes.env` haben alten `CUSTOM_API_KEY` (Rotation nicht gespiegelt). Fix (Host, root): Key aus `/etc/nexifyai/secrets.env` in beide Dateien eintragen, Server neu starten. |
